...
const GetInTouch = () => {
  ...
  const Data = [
    {
      name: "Email",
      detail: userData?.userData.email || "",
      buttonText: "Send Email",
      icon: <MdOutlineEmail className="text-4xl text-[#B7A7FD]" />,
    },
    {
      name: "LinkedIn",
      detail: userData?.portfolio.socialLinks.linkedin || "",
      buttonText: "Connect on LinkedIn",
      icon: <FaLinkedinIn className="text-4xl text-[#B7A7FD]" />,
    },
    {
      name: "GitHub",
      detail: userData?.portfolio.socialLinks.github || "",
      buttonText: "Visit GitHub",
      icon: <FaGithub className="text-4xl text-[#B7A7FD]" />,
    },
    {
      name: "Whatsapp",
      detail: `https://wa.me/${userData?.portfolio?.socialLinks?.mobile}` || "",
      buttonText: "Chat on WhatsApp",
      icon: <FaWhatsapp className="text-5xl text-[#B7A7FD]" />,
    },
  ];
  return (
    <div
      ...
      <div className="cards flex gap-6 mt-12 w-4/5 justify-center flex-wrap px-4">
        {Data.map((item, index) => (
          ...
          >
           ...
           {
            item.detail?( <Link href={item.detail} className="text-black bg-[#B7A7FD] border-2 border-[#B7A7FD] py-2 px-5 rounded-md cursor-pointer flex :hover:bg-[#b7a7fdc2] hover:scale-105 hover:shadow-[0_0_10px_rgba(183,167,253,0.7)] transition duration-300 mt-2">
              {" "}
              {item.buttonText}{" "}
            </Link>):(
               <Link href={"#"} className="text-black bg-[#B7A7FD] border-2 border-[#B7A7FD] py-2 px-5 rounded-md cursor-pointer flex :hover:bg-[#b7a7fdc2] hover:scale-105 hover:shadow-[0_0_10px_rgba(183,167,253,0.7)] transition duration-300 mt-2">
              {" "}
              Soon :)
            </Link>
            )
